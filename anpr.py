...
class Sakura:
	def __init__(self, minAR=2.5, maxAR=5, debug=False):
	...
	def locate_license_plate_candidates(self, gray, keep=5):
		...
		# compute the Scharr gradient representation of the tophat
		...
		# operation, and threshold inv the image using Otsu's method
		...
		# thresholded image
		thresh = cv2.erode(thresh, None, iterations=2)   ##########the tutorial use 2
	...
	def locate_license_plate(self, gray, candidates,
		...
		for c in candidates:
			...
			if ar >= self.minAR and ar <= self.maxAR:
				...
				# plate region
				cv2.imwrite('licenseplate.jpg', licensePlate)
	...
	def build_tesseract_options(self, psm=7):
		...
		# I, O, Z was omitted from the alphanumeric are not use in malaysian number plate
		#alphanumeric = "ABCDEFGHJKLMNPQRSTUVWXY0123456789"
		#options = "-c tessedit_char_whitelist={}".format(alphanumeric)
		options = "-c tessedit_char_whitelist={ABCDEFGHJKLMNPQRSTUVWXY0123456789}"
		options = "-c tessedit_char_blacklist={IOZabcdefghijklmnopqrstuvwxyz~!@$%^&*()_+{}[]<:>?/-}"
		# set the PSM mode
	...
	def check_item_availability(plate_number):
		#connect to db
		conn = pyodbc.connect
